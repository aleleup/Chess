...
public class Pawn extends Piece {
    ...
    public ArrayList<int[]> rangeOfMovement() {
        ...
        int newRowOption = pieceTeam == 1 ? this.pos.getPos()[0] + 1 : this.pos.getPos()[0] - 1;
        if (!this.isCoordInRange(newRowOption)) return res;
        while (i < 2) {
            if (this.isCoordInRange(this.pos.getPos()[1] + i)){
                int[] positionInRange = {newRowOption, this.pos.getPos()[1] + i};
                res.add(positionInRange);
            }
            i++;
        }
        
        if (pieceTeam == 0 && this.pos.getPos()[0] == 6) {
            int[] startingPawnJump = {4, this.pos.getPos()[1]};
            res.add(startingPawnJump);
        }

        if (pieceTeam == 1 && this.pos.getPos()[0] == 1) {
            int[] startingPawnJump = {3, this.pos.getPos()[1]};
            res.add(startingPawnJump);
        }
        return res;
    }
